...
def classify_flight_delays_realtime_response(unique_id):
    """Serves predictions to polling requestors and sends them to Kafka"""

    # Fetch the prediction from MongoDB
    ...
    if prediction:
        ...
        # Replicate the same logic as in the frontend
        prediction_text = get_display_message(prediction["Prediction"])

        # Send the text response to Kafka
        producer.send('flight_delay_classification_response', prediction_text.encode())

        # Include the simplified text in the response
        response["prediction_text"] = prediction_text

    return json_util.dumps(response)


def get_display_message(prediction):
    """Replicates the frontend logic for generating the display message."""
    if prediction == 0 or str(prediction) == '0':
        return "Early (15+ Minutes Early)"
    elif prediction == 1 or str(prediction) == '1':
        return "Slightly Early (0-15 Minute Early)"
    elif prediction == 2 or str(prediction) == '2':
        return "Slightly Late (0-30 Minute Delay)"
    elif prediction == 3 or str(prediction) == '3':
